...
class BlackScholesPriceProcess(PriceProcess):
    ...
    def get_brownian_motions(self, observation_date, requirements, path_count, calibration_params):
        assert isinstance(observation_date, (datetime.date, datetime.datetime)), type(observation_date)
        ...
        if observation_date != fixing_dates[0]:
            raise Exception("Observation date {} ({}) not equal to first fixing date: {} ({})"
                            "".format(observation_date, type(observation_date),
                                      fixing_dates[0], type(fixing_dates[0])))
