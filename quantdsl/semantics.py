    ...
    def apply(self, dsl_globals=None, effective_present_time=None, pending_call_stack=None, is_destacking=False, **dsl_locals):
        ...
        if pending_call_stack and not is_destacking and not 'nostub' in self.decorator_names:
            ...
            pending_call_stack.put(
                stub_id=stub_id,
                call_requirement_id=call_requirement_id,
...
class Lift(DslExpression):
    ...
    def validate(self, args):
        ...
        # Name of a commodity to be lifted.
        self.assert_args_arg(args, posn=0, required_type=(String, Name))

        # Expression to be lifted.
        self.assert_args_arg(args, posn=1, required_type=DslExpression)

        if len(args) == 3:
            # Optional periodization in which to "lift" the expression (day, month, year)
            self.assert_args_arg(args, posn=2, required_type=String)

    @property
    def commodity_name(self):
        """
        Returns a name in the market simulation.
        """
        return self._args[0]
    ...
    def expr(self):
        """
        Returns the DSL expression that is lifted.
        """
        return self._args[1]
    ...
    def mode(self):
        """
        Returns a mode (year, month, or day).
        """
        if len(self._args) == 3:
            return self._args[2]
    ...
    def get_perturbation(self, **kwds):
        ...
        if self.mode:
            ...
            perturbation = json.dumps((commodity_name))
        return perturbation
